module.exports = function(inner, data, path) {
	...
	if(data.reference) {
		...
		for(var i = 0; i < classes.length; i++) {
			...
			fs.writeFileSync(path + "/views/" + name + "/reference/" + class_.file, 
				ejs.render(fs.readFileSync(root + "/views/page.ejs").toString(), data_,
				{
					filename: root + "/views/page.ejs"
				}
			));
			console.log("/views/" + name + "/reference/" + class_.file);
		}
		...
		for(var x = 0; x < reference.length; x++) {
			...
			ensureDirExists(path + "/views/" + name + "/source/" + file.file, name);

			fs.writeFileSync(path + "/views/" + name + "/source/" + file.file, 
				ejs.render(fs.readFileSync(root + "/views/page.ejs").toString(), 
				{
					sidebar: inner.sidebar,
					inner: "./source",
					data: {name: file.name, source: file.source},
					selected: "reference",
					module: name,
					list: 
						{
			...
			console.log("/views/" + name + "/source/" + file.file);
		}
	}
...
function ensureDirExists(file, name) {
